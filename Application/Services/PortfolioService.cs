...
public class PortfolioService : IPortfolioService
{
    ...
    public async Task<List<Project>> GetProjectsAsync(CancellationToken ct = default)
        => await GetOrSetCacheAsync("projects", () => _context.Projects.AsNoTracking().ToListAsync(), ct);

    public async Task<List<Experience>> GetExperiencesAsync(CancellationToken ct = default)
        => await GetOrSetCacheAsync("experiences", () => _context.Experiences.AsNoTracking().OrderByDescending(e => e.Id).ToListAsync(), ct);

    public async Task<List<Skill>> GetSkillsAsync(CancellationToken ct = default)
        => await GetOrSetCacheAsync("skills", () => _context.Skills.AsNoTracking().ToListAsync(), ct);

    public async Task<List<Education>> GetEducationsAsync(CancellationToken ct = default)
        => await GetOrSetCacheAsync("educations", () => _context.Educations.AsNoTracking().ToListAsync(), ct);

    public async Task<List<Certification>> GetCertificationsAsync(CancellationToken ct = default)
        => await GetOrSetCacheAsync("certifications", () => _context.Certifications.AsNoTracking().ToListAsync(), ct);
    ...
    public async Task<PortfolioData> GetAllDataAsync(CancellationToken ct = default)
    {
        ...
        return new PortfolioData(projects, experiences, skills, educations, certifications);
    }
